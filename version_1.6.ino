...
#define M1 3    //Enable pin for the 1st Motor
#define M2 9    //Enable pin for the 2nd Motor
#define motor1_1 4
#define motor1_2 5
...
void left(int mil);
void right(int mil);
void horizontalDetection(int mil);
void diagonalLeft_f(int mil);
void diagonalRight_f(int mil);
void diagonalLeft_b(int mil)
void diagonalRight_b(int mil);
...
static int protothreading_object(struct pt *pt1){
  ...
  while(1){
    ...
    PT_WAIT_WHILE(pt1, millis() <50+ lastTimeChecked && byteIn=='0');
    Serial.println("DE");
...
void loop() {
  ...
  Serial.readBytes(&byteIn, sizeof(byteIn));
...
{ 
    ...
    // The 'alternator' is used to diversify the direction in which the car turns when this function is activated
    int alternator = 0;
